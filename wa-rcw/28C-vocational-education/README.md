---
name: rcw-title-28c
description: Use when the user asks about, cites, quotes, or compares Title 28C RCW — vocational education, workforce training. Triggers on citations like `RCW 28C.`, chapter cites within Title 28C, and subject keywords drawn from the chapter list inside `rules.md`. Do NOT use for: WAC regulations, court rules, federal law, or matters governed by other RCW Titles.
---
...
└── rules.md    ← consolidated text of every chapter in Title 28C (4 chapters, 163,757 bytes)
```
...
grep -n -E '^## RCW 28C\.' rules.md

# Pull a specific section's body (typically 30-90 lines)
grep -n -A 60 -E 'RCW\s+28C\.[0-9]+[A-Z]?[.-][0-9]+' rules.md | head -80
