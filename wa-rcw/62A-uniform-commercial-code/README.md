---
name: rcw-title-62a
description: Use when the user asks about, cites, quotes, or compares Title 62A RCW — the Uniform Commercial Code (UCC) — sales, leases, negotiable instruments, bank collections, letters of credit, secured transactions. Triggers on citations like `RCW 62A.`, chapter cites within Title 62A, and subject keywords drawn from the chapter list inside `rules.md`. Do NOT use for: WAC regulations, court rules, federal law, or matters governed by other RCW Titles.
---
...
└── rules.md    ← consolidated text of every chapter in Title 62A (14 chapters, 1,194,685 bytes)
```
...
grep -n -E '^## RCW 62A\.' rules.md

# Pull a specific section's body (typically 30-90 lines)
grep -n -A 60 -E 'RCW\s+62A\.[0-9]+[A-Z]?[.-][0-9]+' rules.md | head -80
...
- Chapter: `RCW 62A.NN`
- Section: `RCW 62A.NN.MMM`
- UCC numbering uses **hyphenated** section cites — e.g. `RCW 62A.9A-313`, not `RCW 62A.9A.313`. The Title's combined PDF marks chapter boundaries as `Article N` rather than `Chapter X.NN RCW`; we have rewritten those as `## RCW 62A.N` headings for consistency with the rest of the corpus
...
- This Title uses the UCC's `N-MMM` section numbering instead of the standard `NN.MMM`.
