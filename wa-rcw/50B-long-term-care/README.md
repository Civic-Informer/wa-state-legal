---
name: rcw-title-50b
description: Use when the user asks about, cites, quotes, or compares Title 50B RCW — the Long-Term Services and Supports Trust Program (WA Cares). Triggers on citations like `RCW 50B.`, chapter cites within Title 50B, and subject keywords drawn from the chapter list inside `rules.md`. Do NOT use for: WAC regulations, court rules, federal law, or matters governed by other RCW Titles.
---
...
└── rules.md    ← consolidated text of every chapter in Title 50B (1 chapters, 139,453 bytes)
```
...
grep -n -E '^## RCW 50B\.' rules.md

# Pull a specific section's body (typically 30-90 lines)
grep -n -A 60 -E 'RCW\s+50B\.[0-9]+[A-Z]?[.-][0-9]+' rules.md | head -80
