---
name: wa-dupont-municipal-local-rules
description: Use when the user asks about, cites, quotes, or compares DuPont Municipal Court local court rules. Triggers on local-rule citations attached to DuPont (Pierce County). NOTE: DuPont Municipal Court shares its rules document with Lakewood Municipal Court (and 1 other municipal court: Steilacoom). Do NOT use for other WA jurisdictions outside this shared set, state-level rules, RCW, or WAC.
---

# DuPont Municipal Court Local Court Rules
...
DuPont Municipal Court does not publish a municipal-specific local rules document. AOC routes DuPont Municipal Court to the Lakewood Municipal Court rules — the same PDF covers DuPont, Lakewood, Steilacoom. The content of `rules.md` is that shared document verbatim.
...
- `LMCLR GR 1.1 (Lakewood Municipal Court Local Rules, as applied to DuPont Municipal Court)`
- Citation prefixes used in the shared document: LMCLR (Lakewood Municipal Court Local Rules)

## Caveats

- **Shared document.** This file is the Pierce County's Lakewood Municipal Court rules. The rules apply identically to DuPont Municipal Court via AOC routing.
