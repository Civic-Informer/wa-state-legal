---
name: rcw-title-35a
description: Use when the user asks about, cites, quotes, or compares Title 35A RCW — the Optional Municipal Code — alternative governance form available to cities and towns. Triggers on citations like `RCW 35A.`, chapter cites within Title 35A, and subject keywords drawn from the chapter list inside `rules.md`. Do NOT use for: WAC regulations, court rules, federal law, or matters governed by other RCW Titles.
---
...
└── rules.md    ← consolidated text of every chapter in Title 35A (57 chapters, 573,841 bytes)
```
...
grep -n -E '^## RCW 35A\.' rules.md

# Pull a specific section's body (typically 30-90 lines)
grep -n -A 60 -E 'RCW\s+35A\.[0-9]+[A-Z]?[.-][0-9]+' rules.md | head -80
