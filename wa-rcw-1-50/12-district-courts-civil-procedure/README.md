---
name: rcw-title-12
description: Use when the user asks about, cites, quotes, or compares Title 12 RCW — civil procedure in district courts (the lower-court counterpart to Title 4's superior-court procedures). Triggers on citations like `RCW 12.`, chapter cites within Title 12, and subject keywords drawn from the chapter list inside `rules.md`. Do NOT use for: WAC regulations, court rules, federal law, or matters governed by other RCW Titles.
---
...
└── rules.md    ← consolidated text of every chapter in Title 12 (8 chapters, 80,638 bytes)
```
...
grep -n -E '^## RCW 12\.' rules.md

# Pull a specific section's body (typically 30-90 lines)
grep -n -A 60 -E 'RCW\s+12\.[0-9]+[A-Z]?[.-][0-9]+' rules.md | head -80
