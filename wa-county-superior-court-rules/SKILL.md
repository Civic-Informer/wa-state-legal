...
- **Statewide Supreme Court rules (CR, CrR, RAP, ER, RPC, GR), RCW, WAC, and district/municipal court local rules are not in this corpus.** Decline to answer those from this skill; the sibling skills (`wa-state-court-rules/`, `wa-rcw/`, `wa-administrative-code/`, `wa-district-court-rules/`, `wa-municipal-court-rules/`) cover them.
