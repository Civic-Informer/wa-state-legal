---
name: wac-178-economic-development-finance-authority
description: Use when the user asks about, cites, or quotes Title 178 of the Washington Administrative Code (ECONOMIC DEVELOPMENT FINANCE AUTHORITY). Triggers on: 'WAC 178', 'Title 178 WAC', 'chapter 178-NN', 'ECONOMIC DEVELOPMENT FINANCE AUTHORITY'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 178, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 178 WAC — ECONOMIC DEVELOPMENT FINANCE AUTHORITY
...
This is the on-disk archive of Title 178 WAC (ECONOMIC DEVELOPMENT FINANCE AUTHORITY) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 178-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 178-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
178-economic-development-finance-authority/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 178 (2 chapters, 29,132 bytes)
...
grep -n -A 400 '^## WAC 178-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 178-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 178-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 178 WAC`
- Chapter: `chapter 178-NN WAC` or `WAC 178-NN`
- Section: `WAC 178-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 178-NN-NNN` lines followed by the section's title and body, as in the source PDF.
