---
name: wac-220-fish-and-wildlife-department-of
description: Use when the user asks about, cites, or quotes Title 220 of the Washington Administrative Code (FISH AND WILDLIFE, DEPARTMENT OF). Triggers on: 'WAC 220', 'Title 220 WAC', 'chapter 220-NN', 'FISH AND WILDLIFE, DEPARTMENT OF'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 220, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 220 WAC — FISH AND WILDLIFE, DEPARTMENT OF
...
This is the on-disk archive of Title 220 WAC (FISH AND WILDLIFE, DEPARTMENT OF) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 220-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 220-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
220-fish-and-wildlife-department-of/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 220 (56 chapters, 4,056,990 bytes)
...
grep -n -A 400 '^## WAC 220-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 220-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 220-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 220 WAC`
- Chapter: `chapter 220-NN WAC` or `WAC 220-NN`
- Section: `WAC 220-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 220-NN-NNN` lines followed by the section's title and body, as in the source PDF.
