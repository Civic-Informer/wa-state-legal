---
name: wac-414-records-committee-local
description: Use when looking up Title 414 WAC (RECORDS COMMITTEE, LOCAL). NOTE: this title has no active chapters in the 2025 archive — sections were either repealed or recodified elsewhere. Use to confirm a section is no longer in this title.
---

# Title 414 WAC — RECORDS COMMITTEE, LOCAL
...
```
414-records-committee-local/
├── README.md   <- you are here
└── rules.md    <- consolidated chapter section indexes for Title 414 (0 chapters)
```
...
grep -n -A 200 '^## WAC 414-' rules.md | less

# A specific chapter
grep -n -A 200 '^## WAC 414-NN ' rules.md  # replace NN with chapter number

# A specific section by number anywhere in the title
grep -n 'WAC 414-NNN-NNN' rules.md  # replace NNN-NNN with section
```

## Citation format

- Chapter: `chapter 414-NN WAC` or `WAC 414-NN`
- Section: `WAC 414-NN-NNN`
