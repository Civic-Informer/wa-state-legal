---
name: wac-294-land-use-study-commission
description: Use when the user asks about, cites, or quotes Title 294 of the Washington Administrative Code (LAND USE STUDY COMMISSION). Triggers on: 'WAC 294', 'Title 294 WAC', 'chapter 294-NN', 'LAND USE STUDY COMMISSION'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 294, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 294 WAC — LAND USE STUDY COMMISSION
...
This is the on-disk archive of Title 294 WAC (LAND USE STUDY COMMISSION) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 294-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 294-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
294-land-use-study-commission/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 294 (1 chapter, 6,275 bytes)
...
grep -n -A 400 '^## WAC 294-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 294-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 294-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 294 WAC`
- Chapter: `chapter 294-NN WAC` or `WAC 294-NN`
- Section: `WAC 294-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 294-NN-NNN` lines followed by the section's title and body, as in the source PDF.
