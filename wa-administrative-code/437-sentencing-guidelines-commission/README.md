---
name: wac-437-sentencing-guidelines-commission
description: Use when the user asks about, cites, or quotes Title 437 of the Washington Administrative Code (SENTENCING GUIDELINES COMMISSION). Triggers on: 'WAC 437', 'Title 437 WAC', 'chapter 437-NN', 'SENTENCING GUIDELINES COMMISSION'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 437, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 437 WAC — SENTENCING GUIDELINES COMMISSION
...
This is the on-disk archive of Title 437 WAC (SENTENCING GUIDELINES COMMISSION) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 437-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 437-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
437-sentencing-guidelines-commission/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 437 (2 chapters, 15,043 bytes)
...
grep -n -A 400 '^## WAC 437-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 437-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 437-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 437 WAC`
- Chapter: `chapter 437-NN WAC` or `WAC 437-NN`
- Section: `WAC 437-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 437-NN-NNN` lines followed by the section's title and body, as in the source PDF.
