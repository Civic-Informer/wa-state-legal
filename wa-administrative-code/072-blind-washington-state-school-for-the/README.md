---
name: wac-072-blind-washington-state-school-for-the
description: Use when the user asks about, cites, or quotes Title 72 of the Washington Administrative Code (BLIND, WASHINGTON STATE SCHOOL FOR THE). Triggers on: 'WAC 72', 'Title 72 WAC', 'chapter 72-NN', 'BLIND, WASHINGTON STATE SCHOOL FOR THE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 72, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 72 WAC — BLIND, WASHINGTON STATE SCHOOL FOR THE
...
This is the on-disk archive of Title 72 WAC (BLIND, WASHINGTON STATE SCHOOL FOR THE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 72-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 72-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
072-blind-washington-state-school-for-the/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 72 (9 chapters, 158,920 bytes)
...
grep -n -A 400 '^## WAC 72-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 72-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 72-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 72 WAC`
- Chapter: `chapter 72-NN WAC` or `WAC 72-NN`
- Section: `WAC 72-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 72-NN-NNN` lines followed by the section's title and body, as in the source PDF.
