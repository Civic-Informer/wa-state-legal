---
name: wac-090-canvassing-board
description: Use when looking up Title 90 WAC (CANVASSING BOARD). NOTE: this title has no active chapters in the 2025 archive — sections were either repealed or recodified elsewhere. Use to confirm a section is no longer in this title.
---

# Title 90 WAC — CANVASSING BOARD
...
```
090-canvassing-board/
├── README.md   <- you are here
└── rules.md    <- consolidated chapter section indexes for Title 90 (0 chapters)
```
...
grep -n -A 200 '^## WAC 90-' rules.md | less

# A specific chapter
grep -n -A 200 '^## WAC 90-NN ' rules.md  # replace NN with chapter number

# A specific section by number anywhere in the title
grep -n 'WAC 90-NNN-NNN' rules.md  # replace NNN-NNN with section
```

## Citation format

- Chapter: `chapter 90-NN WAC` or `WAC 90-NN`
- Section: `WAC 90-NN-NNN`
