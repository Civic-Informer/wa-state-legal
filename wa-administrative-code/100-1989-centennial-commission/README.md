---
name: wac-100-1989-centennial-commission
description: Use when the user asks about, cites, or quotes Title 100 of the Washington Administrative Code (1989 CENTENNIAL COMMISSION). Triggers on: 'WAC 100', 'Title 100 WAC', 'chapter 100-NN', '1989 CENTENNIAL COMMISSION'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 100, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 100 WAC — 1989 CENTENNIAL COMMISSION
...
This is the on-disk archive of Title 100 WAC (1989 CENTENNIAL COMMISSION) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 100-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 100-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
100-1989-centennial-commission/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 100 (1 chapter, 52,840 bytes)
...
grep -n -A 400 '^## WAC 100-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 100-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 100-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 100 WAC`
- Chapter: `chapter 100-NN WAC` or `WAC 100-NN`
- Section: `WAC 100-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 100-NN-NNN` lines followed by the section's title and body, as in the source PDF.
