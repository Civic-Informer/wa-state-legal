---
name: wac-434-secretary-of-state
description: Use when the user asks about, cites, or quotes Title 434 of the Washington Administrative Code (SECRETARY OF STATE). Triggers on: 'WAC 434', 'Title 434 WAC', 'chapter 434-NN', 'SECRETARY OF STATE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 434, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 434 WAC — SECRETARY OF STATE
...
This is the on-disk archive of Title 434 WAC (SECRETARY OF STATE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 434-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 434-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
434-secretary-of-state/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 434 (42 chapters, 1,027,571 bytes)
...
grep -n -A 400 '^## WAC 434-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 434-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 434-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 434 WAC`
- Chapter: `chapter 434-NN WAC` or `WAC 434-NN`
- Section: `WAC 434-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 434-NN-NNN` lines followed by the section's title and body, as in the source PDF.
