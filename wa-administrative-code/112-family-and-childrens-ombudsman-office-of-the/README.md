---
name: wac-112-family-and-childrens-ombudsman-office-of-the
description: Use when the user asks about, cites, or quotes Title 112 of the Washington Administrative Code (FAMILY AND CHILDREN'S OMBUDSMAN, OFFICE OF THE). Triggers on: 'WAC 112', 'Title 112 WAC', 'chapter 112-NN', 'FAMILY AND CHILDREN'S OMBUDSMAN, OFFICE OF THE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 112, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 112 WAC — FAMILY AND CHILDREN'S OMBUDSMAN, OFFICE OF THE
...
This is the on-disk archive of Title 112 WAC (FAMILY AND CHILDREN'S OMBUDSMAN, OFFICE OF THE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 112-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 112-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
112-family-and-childrens-ombudsman-office-of-the/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 112 (1 chapter, 15,497 bytes)
...
grep -n -A 400 '^## WAC 112-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 112-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 112-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 112 WAC`
- Chapter: `chapter 112-NN WAC` or `WAC 112-NN`
- Section: `WAC 112-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 112-NN-NNN` lines followed by the section's title and body, as in the source PDF.
