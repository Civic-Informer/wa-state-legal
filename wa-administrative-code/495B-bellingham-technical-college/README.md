---
name: wac-495b-bellingham-technical-college
description: Use when the user asks about, cites, or quotes Title 495B of the Washington Administrative Code (BELLINGHAM TECHNICAL COLLEGE). Triggers on: 'WAC 495B', 'Title 495B WAC', 'chapter 495B-NN', 'BELLINGHAM TECHNICAL COLLEGE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 495B, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 495B WAC — BELLINGHAM TECHNICAL COLLEGE
...
This is the on-disk archive of Title 495B WAC (BELLINGHAM TECHNICAL COLLEGE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 495B-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 495B-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
495B-bellingham-technical-college/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 495B (14 chapters, 272,679 bytes)
...
grep -n -A 400 '^## WAC 495B-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 495B-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 495B-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 495B WAC`
- Chapter: `chapter 495B-NN WAC` or `WAC 495B-NN`
- Section: `WAC 495B-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 495B-NN-NNN` lines followed by the section's title and body, as in the source PDF.
