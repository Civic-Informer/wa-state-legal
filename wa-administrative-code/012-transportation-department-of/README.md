---
name: wac-012-transportation-department-of
description: Use when looking up Title 12 WAC (TRANSPORTATION, DEPARTMENT OF). NOTE: this title has no active chapters in the 2025 archive — sections were either repealed or recodified elsewhere. Use to confirm a section is no longer in this title.
---

# Title 12 WAC — TRANSPORTATION, DEPARTMENT OF
...
└── rules.md    <- consolidated chapter section indexes for Title 12 (0 chapters)
```
...
grep -n -A 200 '^## WAC 12-' rules.md | less

# A specific chapter
grep -n -A 200 '^## WAC 12-NN ' rules.md  # replace NN with chapter number

# A specific section by number anywhere in the title
grep -n 'WAC 12-NNN-NNN' rules.md  # replace NNN-NNN with section
```

## Citation format

- Chapter: `chapter 12-NN WAC` or `WAC 12-NN`
- Section: `WAC 12-NN-NNN`
