---
name: wac-249a-higher-education-joint-center-for
description: Use when the user asks about, cites, or quotes Title 249A of the Washington Administrative Code (HIGHER EDUCATION, JOINT CENTER FOR). Triggers on: 'WAC 249A', 'Title 249A WAC', 'chapter 249A-NN', 'HIGHER EDUCATION, JOINT CENTER FOR'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 249A, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 249A WAC — HIGHER EDUCATION, JOINT CENTER FOR
...
This is the on-disk archive of Title 249A WAC (HIGHER EDUCATION, JOINT CENTER FOR) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 249A-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 249A-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
249A-higher-education-joint-center-for/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 249A (1 chapter, 2,504 bytes)
...
grep -n -A 400 '^## WAC 249A-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 249A-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 249A-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 249A WAC`
- Chapter: `chapter 249A-NN WAC` or `WAC 249A-NN`
- Section: `WAC 249A-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 249A-NN-NNN` lines followed by the section's title and body, as in the source PDF.
