---
name: wac-430-washington-state-school-directors-association
description: Use when the user asks about, cites, or quotes Title 430 of the Washington Administrative Code (WASHINGTON STATE SCHOOL DIRECTORS' ASSOCIATION). Triggers on: 'WAC 430', 'Title 430 WAC', 'chapter 430-NN', 'WASHINGTON STATE SCHOOL DIRECTORS' ASSOCIATION'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 430, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 430 WAC — WASHINGTON STATE SCHOOL DIRECTORS' ASSOCIATION
...
This is the on-disk archive of Title 430 WAC (WASHINGTON STATE SCHOOL DIRECTORS' ASSOCIATION) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 430-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 430-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
430-washington-state-school-directors-association/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 430 (1 chapter, 22,899 bytes)
...
grep -n -A 400 '^## WAC 430-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 430-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 430-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 430 WAC`
- Chapter: `chapter 430-NN WAC` or `WAC 430-NN`
- Section: `WAC 430-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 430-NN-NNN` lines followed by the section's title and body, as in the source PDF.
