---
name: wac-196-engineers-and-land-surveyors-board-of-registration-for-professional
description: Use when the user asks about, cites, or quotes Title 196 of the Washington Administrative Code (ENGINEERS AND LAND SURVEYORS, BOARD OF REGISTRATION FOR PROFESSIONAL). Triggers on: 'WAC 196', 'Title 196 WAC', 'chapter 196-NN', 'ENGINEERS AND LAND SURVEYORS, BOARD OF REGISTRATION FOR PROFESSIONAL'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 196, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---
...
This is the on-disk archive of Title 196 WAC (ENGINEERS AND LAND SURVEYORS, BOARD OF REGISTRATION FOR PROFESSIONAL) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 196-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 196-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
196-engineers-and-land-surveyors-board-of-registration-for-professional/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 196 (13 chapters, 205,607 bytes)
...
grep -n -A 400 '^## WAC 196-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 196-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 196-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 196 WAC`
- Chapter: `chapter 196-NN WAC` or `WAC 196-NN`
- Section: `WAC 196-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 196-NN-NNN` lines followed by the section's title and body, as in the source PDF.
