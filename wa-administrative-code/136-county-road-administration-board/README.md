---
name: wac-136-county-road-administration-board
description: Use when the user asks about, cites, or quotes Title 136 of the Washington Administrative Code (COUNTY ROAD ADMINISTRATION BOARD). Triggers on: 'WAC 136', 'Title 136 WAC', 'chapter 136-NN', 'COUNTY ROAD ADMINISTRATION BOARD'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 136, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 136 WAC — COUNTY ROAD ADMINISTRATION BOARD
...
This is the on-disk archive of Title 136 WAC (COUNTY ROAD ADMINISTRATION BOARD) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 136-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 136-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
136-county-road-administration-board/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 136 (31 chapters, 331,651 bytes)
...
grep -n -A 400 '^## WAC 136-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 136-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 136-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 136 WAC`
- Chapter: `chapter 136-NN WAC` or `WAC 136-NN`
- Section: `WAC 136-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 136-NN-NNN` lines followed by the section's title and body, as in the source PDF.
