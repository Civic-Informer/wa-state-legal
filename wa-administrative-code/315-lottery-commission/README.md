---
name: wac-315-lottery-commission
description: Use when the user asks about, cites, or quotes Title 315 of the Washington Administrative Code (LOTTERY COMMISSION). Triggers on: 'WAC 315', 'Title 315 WAC', 'chapter 315-NN', 'LOTTERY COMMISSION'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 315, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 315 WAC — LOTTERY COMMISSION
...
This is the on-disk archive of Title 315 WAC (LOTTERY COMMISSION) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 315-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 315-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
315-lottery-commission/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 315 (14 chapters, 303,401 bytes)
...
grep -n -A 400 '^## WAC 315-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 315-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 315-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 315 WAC`
- Chapter: `chapter 315-NN WAC` or `WAC 315-NN`
- Section: `WAC 315-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 315-NN-NNN` lines followed by the section's title and body, as in the source PDF.
