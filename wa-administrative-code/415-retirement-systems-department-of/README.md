---
name: wac-415-retirement-systems-department-of
description: Use when the user asks about, cites, or quotes Title 415 of the Washington Administrative Code (RETIREMENT SYSTEMS, DEPARTMENT OF). Triggers on: 'WAC 415', 'Title 415 WAC', 'chapter 415-NN', 'RETIREMENT SYSTEMS, DEPARTMENT OF'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 415, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 415 WAC — RETIREMENT SYSTEMS, DEPARTMENT OF
...
This is the on-disk archive of Title 415 WAC (RETIREMENT SYSTEMS, DEPARTMENT OF) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 415-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 415-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
415-retirement-systems-department-of/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 415 (21 chapters, 1,706,913 bytes)
...
grep -n -A 400 '^## WAC 415-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 415-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 415-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 415 WAC`
- Chapter: `chapter 415-NN WAC` or `WAC 415-NN`
- Section: `WAC 415-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 415-NN-NNN` lines followed by the section's title and body, as in the source PDF.
