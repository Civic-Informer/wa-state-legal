---
name: wac-131-community-and-technical-colleges-board-for
description: Use when the user asks about, cites, or quotes Title 131 of the Washington Administrative Code (COMMUNITY AND TECHNICAL COLLEGES, BOARD FOR). Triggers on: 'WAC 131', 'Title 131 WAC', 'chapter 131-NN', 'COMMUNITY AND TECHNICAL COLLEGES, BOARD FOR'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 131, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---
...
This is the on-disk archive of Title 131 WAC (COMMUNITY AND TECHNICAL COLLEGES, BOARD FOR) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 131-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 131-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
131-community-and-technical-colleges-board-for/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 131 (11 chapters, 167,440 bytes)
...
grep -n -A 400 '^## WAC 131-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 131-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 131-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 131 WAC`
- Chapter: `chapter 131-NN WAC` or `WAC 131-NN`
- Section: `WAC 131-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 131-NN-NNN` lines followed by the section's title and body, as in the source PDF.
