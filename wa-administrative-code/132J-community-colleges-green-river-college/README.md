---
name: wac-132j-community-colleges-green-river-college
description: Use when the user asks about, cites, or quotes Title 132J of the Washington Administrative Code (COMMUNITY COLLEGES-GREEN RIVER COLLEGE). Triggers on: 'WAC 132J', 'Title 132J WAC', 'chapter 132J-NN', 'COMMUNITY COLLEGES-GREEN RIVER COLLEGE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 132J, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 132J WAC — COMMUNITY COLLEGES-GREEN RIVER COLLEGE
...
This is the on-disk archive of Title 132J WAC (COMMUNITY COLLEGES-GREEN RIVER COLLEGE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 132J-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 132J-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
132J-community-colleges-green-river-college/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 132J (9 chapters, 192,899 bytes)
...
grep -n -A 400 '^## WAC 132J-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 132J-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 132J-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 132J WAC`
- Chapter: `chapter 132J-NN WAC` or `WAC 132J-NN`
- Section: `WAC 132J-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 132J-NN-NNN` lines followed by the section's title and body, as in the source PDF.
