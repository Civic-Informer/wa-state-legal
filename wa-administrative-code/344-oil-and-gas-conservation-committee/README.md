---
name: wac-344-oil-and-gas-conservation-committee
description: Use when the user asks about, cites, or quotes Title 344 of the Washington Administrative Code (OIL AND GAS CONSERVATION COMMITTEE). Triggers on: 'WAC 344', 'Title 344 WAC', 'chapter 344-NN', 'OIL AND GAS CONSERVATION COMMITTEE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 344, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 344 WAC — OIL AND GAS CONSERVATION COMMITTEE
...
This is the on-disk archive of Title 344 WAC (OIL AND GAS CONSERVATION COMMITTEE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 344-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 344-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
344-oil-and-gas-conservation-committee/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 344 (3 chapters, 103,014 bytes)
...
grep -n -A 400 '^## WAC 344-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 344-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 344-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 344 WAC`
- Chapter: `chapter 344-NN WAC` or `WAC 344-NN`
- Section: `WAC 344-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 344-NN-NNN` lines followed by the section's title and body, as in the source PDF.
