---
name: wac-143-consolidated-technology-services
description: Use when the user asks about, cites, or quotes Title 143 of the Washington Administrative Code (CONSOLIDATED TECHNOLOGY SERVICES). Triggers on: 'WAC 143', 'Title 143 WAC', 'chapter 143-NN', 'CONSOLIDATED TECHNOLOGY SERVICES'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 143, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 143 WAC — CONSOLIDATED TECHNOLOGY SERVICES
...
This is the on-disk archive of Title 143 WAC (CONSOLIDATED TECHNOLOGY SERVICES) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 143-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 143-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
143-consolidated-technology-services/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 143 (2 chapters, 21,128 bytes)
...
grep -n -A 400 '^## WAC 143-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 143-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 143-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 143 WAC`
- Chapter: `chapter 143-NN WAC` or `WAC 143-NN`
- Section: `WAC 143-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 143-NN-NNN` lines followed by the section's title and body, as in the source PDF.
