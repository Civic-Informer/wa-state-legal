---
name: wac-289-corrections-standards-board
description: Use when looking up Title 289 WAC (CORRECTIONS STANDARDS BOARD). NOTE: this title has no active chapters in the 2025 archive — sections were either repealed or recodified elsewhere. Use to confirm a section is no longer in this title.
---

# Title 289 WAC — CORRECTIONS STANDARDS BOARD
...
```
289-corrections-standards-board/
├── README.md   <- you are here
└── rules.md    <- consolidated chapter section indexes for Title 289 (0 chapters)
```
...
grep -n -A 200 '^## WAC 289-' rules.md | less

# A specific chapter
grep -n -A 200 '^## WAC 289-NN ' rules.md  # replace NN with chapter number

# A specific section by number anywhere in the title
grep -n 'WAC 289-NNN-NNN' rules.md  # replace NNN-NNN with section
```

## Citation format

- Chapter: `chapter 289-NN WAC` or `WAC 289-NN`
- Section: `WAC 289-NN-NNN`
