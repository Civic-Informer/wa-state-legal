---
name: wac-025-archaeology-and-historic-preservation
description: Use when the user asks about, cites, or quotes Title 25 of the Washington Administrative Code (ARCHAEOLOGY AND HISTORIC PRESERVATION,). Triggers on: 'WAC 25', 'Title 25 WAC', 'chapter 25-NN', 'ARCHAEOLOGY AND HISTORIC PRESERVATION,'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 25, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 25 WAC — ARCHAEOLOGY AND HISTORIC PRESERVATION,
...
This is the on-disk archive of Title 25 WAC (ARCHAEOLOGY AND HISTORIC PRESERVATION,) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 25-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 25-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
025-archaeology-and-historic-preservation/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 25 (5 chapters, 140,524 bytes)
...
grep -n -A 400 '^## WAC 25-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 25-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 25-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 25 WAC`
- Chapter: `chapter 25-NN WAC` or `WAC 25-NN`
- Section: `WAC 25-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 25-NN-NNN` lines followed by the section's title and body, as in the source PDF.
