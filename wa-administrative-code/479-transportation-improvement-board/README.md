---
name: wac-479-transportation-improvement-board
description: Use when the user asks about, cites, or quotes Title 479 of the Washington Administrative Code (TRANSPORTATION IMPROVEMENT BOARD). Triggers on: 'WAC 479', 'Title 479 WAC', 'chapter 479-NN', 'TRANSPORTATION IMPROVEMENT BOARD'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 479, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 479 WAC — TRANSPORTATION IMPROVEMENT BOARD
...
This is the on-disk archive of Title 479 WAC (TRANSPORTATION IMPROVEMENT BOARD) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 479-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 479-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
479-transportation-improvement-board/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 479 (6 chapters, 135,630 bytes)
...
grep -n -A 400 '^## WAC 479-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 479-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 479-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 479 WAC`
- Chapter: `chapter 479-NN WAC` or `WAC 479-NN`
- Section: `WAC 479-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 479-NN-NNN` lines followed by the section's title and body, as in the source PDF.
