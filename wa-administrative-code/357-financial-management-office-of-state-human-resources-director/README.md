---
name: wac-357-financial-management-office-of-state-human-resources-director
description: Use when the user asks about, cites, or quotes Title 357 of the Washington Administrative Code (FINANCIAL MANAGEMENT, OFFICE OF-STATE HUMAN RESOURCES DIRECTOR). Triggers on: 'WAC 357', 'Title 357 WAC', 'chapter 357-NN', 'FINANCIAL MANAGEMENT, OFFICE OF-STATE HUMAN RESOURCES DIRECTOR'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 357, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---
...
This is the on-disk archive of Title 357 WAC (FINANCIAL MANAGEMENT, OFFICE OF-STATE HUMAN RESOURCES DIRECTOR) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 357-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 357-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
357-financial-management-office-of-state-human-resources-director/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 357 (18 chapters, 881,804 bytes)
...
grep -n -A 400 '^## WAC 357-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 357-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 357-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 357 WAC`
- Chapter: `chapter 357-NN WAC` or `WAC 357-NN`
- Section: `WAC 357-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 357-NN-NNN` lines followed by the section's title and body, as in the source PDF.
