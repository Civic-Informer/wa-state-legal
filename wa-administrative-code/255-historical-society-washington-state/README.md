---
name: wac-255-historical-society-washington-state
description: Use when the user asks about, cites, or quotes Title 255 of the Washington Administrative Code (HISTORICAL SOCIETY, WASHINGTON STATE). Triggers on: 'WAC 255', 'Title 255 WAC', 'chapter 255-NN', 'HISTORICAL SOCIETY, WASHINGTON STATE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 255, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 255 WAC — HISTORICAL SOCIETY, WASHINGTON STATE
...
This is the on-disk archive of Title 255 WAC (HISTORICAL SOCIETY, WASHINGTON STATE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 255-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 255-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
255-historical-society-washington-state/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 255 (3 chapters, 37,947 bytes)
...
grep -n -A 400 '^## WAC 255-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 255-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 255-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 255 WAC`
- Chapter: `chapter 255-NN WAC` or `WAC 255-NN`
- Section: `WAC 255-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 255-NN-NNN` lines followed by the section's title and body, as in the source PDF.
