---
name: wac-132u-community-colleges-whatcom-community-college
description: Use when the user asks about, cites, or quotes Title 132U of the Washington Administrative Code (COMMUNITY COLLEGES-WHATCOM COMMUNITY COLLEGE). Triggers on: 'WAC 132U', 'Title 132U WAC', 'chapter 132U-NN', 'COMMUNITY COLLEGES-WHATCOM COMMUNITY COLLEGE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 132U, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 132U WAC — COMMUNITY COLLEGES-WHATCOM COMMUNITY COLLEGE
...
This is the on-disk archive of Title 132U WAC (COMMUNITY COLLEGES-WHATCOM COMMUNITY COLLEGE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 132U-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 132U-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
132U-community-colleges-whatcom-community-college/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 132U (14 chapters, 210,404 bytes)
...
grep -n -A 400 '^## WAC 132U-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 132U-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 132U-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 132U WAC`
- Chapter: `chapter 132U-NN WAC` or `WAC 132U-NN`
- Section: `WAC 132U-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 132U-NN-NNN` lines followed by the section's title and body, as in the source PDF.
