---
name: wac-018-air-pollution
description: Use when looking up Title 18 WAC (AIR POLLUTION). NOTE: this title has no active chapters in the 2025 archive — sections were either repealed or recodified elsewhere. Use to confirm a section is no longer in this title.
---
...
└── rules.md    <- consolidated chapter section indexes for Title 18 (0 chapters)
```
...
grep -n -A 200 '^## WAC 18-' rules.md | less

# A specific chapter
grep -n -A 200 '^## WAC 18-NN ' rules.md  # replace NN with chapter number

# A specific section by number anywhere in the title
grep -n 'WAC 18-NNN-NNN' rules.md  # replace NNN-NNN with section
```

## Citation format

- Chapter: `chapter 18-NN WAC` or `WAC 18-NN`
- Section: `WAC 18-NN-NNN`
