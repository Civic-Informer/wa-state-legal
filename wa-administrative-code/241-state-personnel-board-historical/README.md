---
name: wac-241-state-personnel-board-historical
description: Use when the user asks about, cites, or quotes Title 241 of the Washington Administrative Code (STATE PERSONNEL BOARD (historical; no active chapters; only Public Records remains)). Triggers on: 'WAC 241', 'Title 241 WAC', 'chapter 241-NN', 'STATE PERSONNEL BOARD (historical; no active chapters; only Public Records remains)'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 241, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 241 WAC — STATE PERSONNEL BOARD (historical; no active chapters; only Public Records remains)
...
This is the on-disk archive of Title 241 WAC (STATE PERSONNEL BOARD (historical; no active chapters; only Public Records remains)) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 241-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 241-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
241-state-personnel-board-historical/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 241 (1 chapter, 4,234 bytes)
...
grep -n -A 400 '^## WAC 241-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 241-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 241-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 241 WAC`
- Chapter: `chapter 241-NN WAC` or `WAC 241-NN`
- Section: `WAC 241-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 241-NN-NNN` lines followed by the section's title and body, as in the source PDF.
