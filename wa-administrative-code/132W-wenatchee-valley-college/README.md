---
name: wac-132w-wenatchee-valley-college
description: Use when the user asks about, cites, or quotes Title 132W of the Washington Administrative Code (WENATCHEE VALLEY COLLEGE). Triggers on: 'WAC 132W', 'Title 132W WAC', 'chapter 132W-NN', 'WENATCHEE VALLEY COLLEGE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 132W, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---
...
This is the on-disk archive of Title 132W WAC (WENATCHEE VALLEY COLLEGE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 132W-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 132W-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
132W-wenatchee-valley-college/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 132W (15 chapters, 197,785 bytes)
...
grep -n -A 400 '^## WAC 132W-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 132W-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 132W-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 132W WAC`
- Chapter: `chapter 132W-NN WAC` or `WAC 132W-NN`
- Section: `WAC 132W-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 132W-NN-NNN` lines followed by the section's title and body, as in the source PDF.
