---
name: wac-148-deaf-and-hard-of-hearing-youth-center-for
description: Use when the user asks about, cites, or quotes Title 148 of the Washington Administrative Code (DEAF AND HARD OF HEARING YOUTH, CENTER FOR). Triggers on: 'WAC 148', 'Title 148 WAC', 'chapter 148-NN', 'DEAF AND HARD OF HEARING YOUTH, CENTER FOR'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 148, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 148 WAC — DEAF AND HARD OF HEARING YOUTH, CENTER FOR
...
This is the on-disk archive of Title 148 WAC (DEAF AND HARD OF HEARING YOUTH, CENTER FOR) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 148-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 148-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
148-deaf-and-hard-of-hearing-youth-center-for/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 148 (9 chapters, 187,517 bytes)
...
grep -n -A 400 '^## WAC 148-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 148-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 148-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 148 WAC`
- Chapter: `chapter 148-NN WAC` or `WAC 148-NN`
- Section: `WAC 148-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 148-NN-NNN` lines followed by the section's title and body, as in the source PDF.
