---
name: wac-132c-olympic-college
description: Use when the user asks about, cites, or quotes Title 132C of the Washington Administrative Code (OLYMPIC COLLEGE). Triggers on: 'WAC 132C', 'Title 132C WAC', 'chapter 132C-NN', 'OLYMPIC COLLEGE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 132C, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 132C WAC — OLYMPIC COLLEGE
...
This is the on-disk archive of Title 132C WAC (OLYMPIC COLLEGE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 132C-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 132C-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
132C-olympic-college/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 132C (6 chapters, 173,069 bytes)
...
grep -n -A 400 '^## WAC 132C-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 132C-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 132C-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 132C WAC`
- Chapter: `chapter 132C-NN WAC` or `WAC 132C-NN`
- Section: `WAC 132C-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 132C-NN-NNN` lines followed by the section's title and body, as in the source PDF.
