---
name: wac-067-blind-department-of-services-for-the
description: Use when the user asks about, cites, or quotes Title 67 of the Washington Administrative Code (BLIND, DEPARTMENT OF SERVICES FOR THE). Triggers on: 'WAC 67', 'Title 67 WAC', 'chapter 67-NN', 'BLIND, DEPARTMENT OF SERVICES FOR THE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 67, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 67 WAC — BLIND, DEPARTMENT OF SERVICES FOR THE
...
This is the on-disk archive of Title 67 WAC (BLIND, DEPARTMENT OF SERVICES FOR THE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 67-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 67-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
067-blind-department-of-services-for-the/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 67 (6 chapters, 361,634 bytes)
...
grep -n -A 400 '^## WAC 67-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 67-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 67-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 67 WAC`
- Chapter: `chapter 67-NN WAC` or `WAC 67-NN`
- Section: `WAC 67-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 67-NN-NNN` lines followed by the section's title and body, as in the source PDF.
