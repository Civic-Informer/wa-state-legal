---
name: wac-495a-bates-technical-college
description: Use when the user asks about, cites, or quotes Title 495A of the Washington Administrative Code (BATES TECHNICAL COLLEGE). Triggers on: 'WAC 495A', 'Title 495A WAC', 'chapter 495A-NN', 'BATES TECHNICAL COLLEGE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 495A, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 495A WAC — BATES TECHNICAL COLLEGE
...
This is the on-disk archive of Title 495A WAC (BATES TECHNICAL COLLEGE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 495A-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 495A-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
495A-bates-technical-college/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 495A (14 chapters, 209,350 bytes)
...
grep -n -A 400 '^## WAC 495A-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 495A-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 495A-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 495A WAC`
- Chapter: `chapter 495A-NN WAC` or `WAC 495A-NN`
- Section: `WAC 495A-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 495A-NN-NNN` lines followed by the section's title and body, as in the source PDF.
