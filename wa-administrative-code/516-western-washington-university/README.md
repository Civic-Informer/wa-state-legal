---
name: wac-516-western-washington-university
description: Use when the user asks about, cites, or quotes Title 516 of the Washington Administrative Code (WESTERN WASHINGTON UNIVERSITY). Triggers on: 'WAC 516', 'Title 516 WAC', 'chapter 516-NN', 'WESTERN WASHINGTON UNIVERSITY'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 516, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---

# Title 516 WAC — WESTERN WASHINGTON UNIVERSITY
...
This is the on-disk archive of Title 516 WAC (WESTERN WASHINGTON UNIVERSITY) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 516-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 516-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
├── rules.md          <- full body text of every section in Title 516 (25 chapters, 354,174 bytes)
...
grep -n -A 400 '^## WAC 516-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 516-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 516-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 516 WAC`
- Chapter: `chapter 516-NN WAC` or `WAC 516-NN`
- Section: `WAC 516-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 516-NN-NNN` lines followed by the section's title and body, as in the source PDF.
