---
name: wac-132h-bellevue-college
description: Use when the user asks about, cites, or quotes Title 132H of the Washington Administrative Code (BELLEVUE COLLEGE). Triggers on: 'WAC 132H', 'Title 132H WAC', 'chapter 132H-NN', 'BELLEVUE COLLEGE'. `rules.md` contains the full body text of every section in this Title, sourced from the 2025 archive's per-chapter `COMBINEDCHAPTER.pdf` files. Do NOT use for WAC titles other than 132H, for RCW (Revised Code of Washington) statutes, or for federal regulations.
---
...
This is the on-disk archive of Title 132H WAC (BELLEVUE COLLEGE) from the Washington State Legislature's 2025 WAC Archive. The consolidated `rules.md` contains the **full body text** of every section in every chapter of this Title, ordered by chapter cite.

Each `## WAC 132H-NN — CHAPTER NAME` heading is followed by the cleaned plain-text body of that chapter's combined PDF (the same PDF the legislature publishes for download from each chapter's page). Section subheadings appear in the text as the original PDF presented them (e.g. `WAC 132H-NN-NNN` followed by section title and body, with statutory-authority history brackets at the end of each section).
...
```
132H-bellevue-college/
├── README.md         <- you are here
├── rules.md          <- full body text of every section in Title 132H (15 chapters, 311,786 bytes)
...
grep -n -A 400 '^## WAC 132H-NN ' rules.md  # replace NN with chapter number

# A specific section by full citation
grep -n -B 2 -A 80 'WAC 132H-NN-NNN' rules.md  # replace NN-NNN

# All section short titles (cite + first line)
grep -nE '^WAC 132H-[0-9A-Z]+-[0-9A-Z]+' rules.md
```

## Citation format

- Title: `Title 132H WAC`
- Chapter: `chapter 132H-NN WAC` or `WAC 132H-NN`
- Section: `WAC 132H-NN-NNN`
...
- Section subheadings inside chapters are inline with the body text (not separate `###` markdown headings) — they appear as `WAC 132H-NN-NNN` lines followed by the section's title and body, as in the source PDF.
