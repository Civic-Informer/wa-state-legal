...
**Source:** https://www.courts.wa.gov/court_rules/pdf/LCR/22/DIS/LCR_Lincoln_DIS.pdf  (provenance only — do not refetch)
...
```bash
grep -n -E '^\s*(LARLJ|LCrRLJ|LIRLJ) ?[0-9]' rules.md
```

## Citation examples

- `Lincoln County District Court LARLJ {rule number}`
- `Lincoln County District Court LCrRLJ {rule number}`
- `Lincoln County District Court LIRLJ {rule number}`
